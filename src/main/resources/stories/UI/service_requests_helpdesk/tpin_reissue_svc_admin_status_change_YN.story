...
Given user is logged in institution
When user updates the wallet admin status to MANUAL BLOCK 1 [4]
...
Scenario: 4 To verify that the USER_DELETE is set to 'YN' in the TPIN_DOWNLOAD if the admin status is set to normal
Given user is logged in institution
When user updates the wallet admin status to NORMAL [0]
And reissue TPIN dump download batch is processed for credit
Then verify that the USER_DELETE field is set to YN in the TPIN reissue DAT file