...
public class MCCRulePlanPage extends AbstractBasePage {
	...
	public void editMCCRulePlan(DevicePlan device, Program program) {
		logger.info("Edit MCC Rule Plan {}", program.getMccRulePlan());
		WebElementUtils.enterText(planCodeSearchTxt, program.getMccRulePlan());
		clickSearchButton();
		editFirstRecord();
		runWithinPopup("Edit MCC Rule Plan", () -> {
			SimulatorUtilities.wait(1500);
			WebElementUtils.elementToBeClickable(fromMccCodeTxt);
			WebElementUtils.enterText(fromMccCodeTxt, "5999");
			SimulatorUtilities.wait(100);
			WebElementUtils.enterText(toMccCodeTxt, "5999");
			clickSearchButton();
			waitForPageToLoad(driver());
