...
public class NetworkMembershipPage extends AbstractModelPage {
	...
	public void clickAddNetworkMemberShip() {
		waitForElementVisible(addNetworkMembership);
	...
	public void selectInterchangeType(DeviceCreation devicecreation) {
		waitForElementVisible(Interchange);
		selectByVisibleText(Interchange, devicecreation.getInterchange());
	}

	public void fillPresentTime() {
		waitForElementVisible(PresentmentTimeLimit);
		enterText(PresentmentTimeLimit, NetworkMembershipDataProvider().getPresentmentTimeLimit());
	}
	...
	public void selectCutoverMinutes() {
		waitForElementVisible(CutoverMins);
		SelectDropDownByIndex(CutoverMins, 2);
		addWicketAjaxListeners(getFinder().getWebDriver());
	}

	public void selectSettlementCurrency() {
		waitForElementVisible(SettlementCurrency);
		selectByVisibleText(SettlementCurrency, NetworkMembershipDataProvider().getPresentmentTimeLimit());
		addWicketAjaxListeners(getFinder().getWebDriver());
	}
	...
	public void addnetworkmembership(String interchangeType, String presentTimelimit, String settlementCurrency) {
		...
		addWicketAjaxListeners(getFinder().getWebDriver());
		switchToIframe(Constants.ADD_NETWORK_MEMBERSHIP_FRAME);
		selectByVisibleText(Interchange, interchangeType);
		enterText(PresentmentTimeLimit, presentTimelimit);
		SelectDropDownByIndex(CutoverHours, 1);
		addWicketAjaxListeners(getFinder().getWebDriver());
		waitForElementVisible(CutoverHours);
		SelectDropDownByIndex(CutoverMins, 2);
		selectByVisibleText(SettlementCurrency, settlementCurrency);
		ClickButton(save);
	...
	public void editnetworkmembership(String interchange) {
		...
		addWicketAjaxListeners(getFinder().getWebDriver());
		selectByVisibleText(CutoverHours, MapUtils.fnGetInputDataFromMap("CutOverHours"));
		addWicketAjaxListeners(getFinder().getWebDriver());
		selectByVisibleText(CutoverMins, MapUtils.fnGetInputDataFromMap("CutOverMins"));
		addWicketAjaxListeners(getFinder().getWebDriver());
		ClickButton(save);
		addWicketAjaxListeners(getFinder().getWebDriver());
	...
	public String checkErrormessage() {
		// String strOutputMessage = PanelError.getText().split("\\n")[0];
	...
	public boolean verifyNetworkMembership(String network) {
		...
		addWicketAjaxListeners(getFinder().getWebDriver());
		switchToIframe(Constants.ADD_NETWORK_MEMBERSHIP_FRAME);
		List<WebElement> drpDwnOptions = Interchange.getSelect().getOptions();
	...
	public void setAddNetworkMembership(MCWebElement addNetworkMembership) {
		this.addNetworkMembership = addNetworkMembership;
	}
	public void verifyUiOperationStatus() {
		logger.info("Network Membership");
		verifyUiOperation("Add Network Membership");
