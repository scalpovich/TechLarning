...
public class MiscUtils {
	...
	{
		return ("AUTO" + MiscUtils.generateRandomNumberBetween2Number(10, 99) + MiscUtils.randomAlphabet(4)).toUpperCase(); 		
	}

	public static String generate8CharAlphaNumeric()
	{
		return ("AUTO" + MiscUtils.generateRandomNumberBetween2Number(10, 99) + MiscUtils.randomAlphabet(2)).toUpperCase(); 		
	}
	...
	/**
	 * Iterates through String constants defined in class and 
	 * returns value which starts with prefix ignoring case.
	 * @param cls
	 * @param prefix
	 * @return constant string value
	 */
	public static String getConstantStringFromClassByPefixMatch(Class<?> cls, String prefix) {
		Predicate<Integer> isConstant = modifiers -> Modifier.isPublic(modifiers)
	...
	public static Map<String, String> namedGroups(Matcher matcher) {
		...
		return indexes.entrySet().stream().collect(Collectors.toMap(Entry::getKey,
