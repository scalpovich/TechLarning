...
public class MdfsDetailsKeyValuePair {
	...
	private static void setMdfsInstallationVersionData() {
		...
		mdfsInstallationVersionKeyValuePair.put(MDFS_18_Q2, "C://Program Files (x86)//MasterCard//OTP 7.32.1016//Bin//MSPMCW.exe");
		mdfsInstallationVersionKeyValuePair.put(MDFS_17_Q4, "C://Program Files (x86)//MasterCard//OTP 7.32.1014//Bin//MSPMCW1741.exe");
	...
	public static void getLatestVersionMdfsDetailsInstalledOnMachine() {
		...
		for (Map.Entry<String,String> entry : mdfsInstallationVersionKeyValuePair.entrySet()) {
			MiscUtils.reportToConsole("MDFS Exe being searched for <--> versionNumber = " + entry.getKey() + " ---  exePath = " + entry.getValue());
	...
	public static void getSpecificMdfsVersionDetails(String version) { 
		...
		for (Map.Entry<String,String> entry : mdfsInstallationVersionKeyValuePair.entrySet()) {
			...
			MiscUtils.reportToConsole("MDFS Exe being searched for <--> version.toLowerCase().trim() = " + version.toLowerCase().trim() + " : : " + entry.getKey().toLowerCase().trim());
			MiscUtils.reportToConsole("MDFS Exe being searched for <--> versionNumber = " + entry.getKey() + " ---  exePath = " + entry.getValue());
