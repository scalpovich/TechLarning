...
public class JsonUtilitySteps {
	...
	public void mappingJsonValuesInExcel(String product) throws Exception {
		...
		/**
		 * Step-1 creating context of excel and JSON for all the fields mention
		 * in InstitutionData.json
		 */
		...
		/**
		 * Step-2 Getting the initial excel context which retrieves all the data
		 * of @Storyname
		 */
	...
	public void updatedExcelContext(Map<String, String> jsonMap, Map<String, Object> map) {
		String updatedValue = "";
	...
	public void replacingValuesFromJsonToExcel(Method[] methodsJson, Method[] methodsExcel, Map<String, String> jsonMap)
		...
		for (int i = 0; i < methodsJson.length; i++) {
			...
			if (methodsJson[i].getName().startsWith("get")) {
				valueJson = (String) jsonMappingData.getClass().getMethod(methodsJson[i].getName())
						.invoke(jsonMappingData);

				for (int j = 0; j < methodsExcel.length; j++) {
					if (methodsJson[i].getName().equalsIgnoreCase(methodsExcel[j].getName())
							&& methodsJson[i].getName().startsWith("get")) {

						creditMappingForExcel = context.get(CreditConstants.EXCEL_VALUES);

						valueExcel = (String) creditMappingForExcel.getClass().getMethod(methodsExcel[j].getName())
								.invoke(creditMappingForExcel);
	...
	public void contextForExcelAndJson(String institution) {
		if (StringUtils.isEmpty(institution)) {
			jsonMappingData = InstitutionData.createWithProvider(provider,
					Institution.createWithProvider(provider).getCode());
	...
	public Map<String, String> getJsonMap(Method[] getMap) throws Exception {
		...
		for (int i = 0; i < getMap.length; i++) {
			if (getMap[i].getName().startsWith("get")) {
				...
				tempMap.put(getMap[i].getName().toUpperCase(),
						(String) jsonMappingData.getClass().getMethod(getMap[i].getName()).invoke(jsonMappingData));
			}
		}
		return tempMap;
	}
