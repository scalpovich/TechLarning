...
public class WebElementUtils {
	...
	public static void switchToChildWindowByTitleAndCloseParent(WebDriver driver, String title) {
		...
		String targetWindow = new FluentWait<WebDriver>(driver)
				.until((com.google.common.base.Function<WebDriver, String>) wd -> wd.getWindowHandles()
						.stream()
						.filter(handle -> wd.switchTo().window(handle)
	...
	public static void switchToParentWindowByHandle(WebDriver driver,
	...
	public static List<String> getOptionsTextFromSelect(MCWebElement element) {
		return asWebElement(element).findElements(By.tagName("option")).stream()
	...
	public static void runWithinFrame(WebDriver driver, long timeoutInSec,
			By frameLocator, Runnable action) {
	...
	public static MCWebElement findElement(MCWebElement context, By locator) {
		if (locator instanceof By.ByXPath) {
			...
			locator = new ByNativeXPath(xPath);
		}
		
		FluentWebElement fluent = context.getFluent().element(locator);
		try {
