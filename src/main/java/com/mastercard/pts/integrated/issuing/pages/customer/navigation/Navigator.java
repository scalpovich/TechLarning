...
public final class Navigator extends AbstractBasePage {
	...
	public <T extends AbstractPage> T navigateToPage(String pageName) {
		return navigateToPage((Class<T>) pageObjFactory.getPage(pageName)
				.getClass());
	}
	 ...
	 * @return
	 */
	public <T extends AbstractPage> boolean isPageAvailable(
	...
	/**
	 * Method navigates to page by page name and checks if page is available
	 * E.g. Page Object should have following annotation
	 * 
	 * @Navigation (tabTitle = AdministrationNav.TAB_TITLE , menuItemsTree
	 *             ={AdministrationNav.Setup, AdministrationNav._Role})
	...
	private void clickNode(String treeMenuItem) {
		By locator = By.id(treeMenuItem);
		WebElement node = fluentWait(() -> driverProvider.get()
				.findElement(locator));
		if (!hasClass(node, "p1-active")
				&& !hasClass(node, "p2-active")) {
	...
	public boolean isNavigationPathAvailable(String tabTitle,
		...
		return isNavigationPathAvailable(tabTitle)
				&& isNavigationPathAvailable(treeMenuItems);
	}
	...
	private void clickElement(By locator) {
		retryUntilNoErrors(() -> driverProvider.get()
