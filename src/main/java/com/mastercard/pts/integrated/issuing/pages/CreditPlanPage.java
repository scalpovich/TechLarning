...
public class CreditPlanPage extends AbstractBasePage {
	...
	public void addcreditplan() {
		...
		Description.sendKeys(env
				.getProperty("is.dinners.creditplan.Description"));
		Abbreviation.sendKeys(env
				.getProperty("is.dinners.creditplan.Abbreviation"));
		CustomUtils.ThreadDotSleep(1000);
		PaymentDate.getSelect().selectByVisibleText(
				env.getProperty("is.dinners.creditplan.PaymentDate"));
		CustomUtils.ThreadDotSleep(1000);
		PaymentDateDays.sendKeys(env
				.getProperty("is.dinners.creditplan.PaymentDateDays"));
		CustomUtils.ThreadDotSleep(1000);
		UnpaidDate.getSelect().selectByVisibleText(
				env.getProperty("is.dinners.creditplan.UnpaidDate"));
		CustomUtils.ThreadDotSleep(1000);
		UnpaidDateDays.sendKeys(env
				.getProperty("is.dinners.creditplan.UnpaidDateDays"));
		CustomUtils.ThreadDotSleep(1000);
		...
		CustomUtils.ThreadDotSleep(1000);
		MinimumDue
				.sendKeys(env.getProperty("is.dinners.creditplan.MinimumDue"));
		CustomUtils.ThreadDotSleep(1000);
		...
		CustomUtils.ThreadDotSleep(1000);
		AllowedPercentage.sendKeys(env
				.getProperty("is.dinners.creditplan.AllowedPercentage"));
