...
public class DeviceCreateDevicePage extends AbstractModelPage {

	private static final Logger logger = LoggerFactory
			.getLogger(DeviceCreateDevicePage.class);
	...
	public void selectAppliedForProduct(String product) {
		WebElementUtils.selectDropDownByVisibleText(appliedForProdutDDwn,
				product);
	}
	...
	protected Collection<ExpectedCondition<WebElement>> isLoadedConditions() {
		return Arrays.asList(WebElementUtils
				.visibilityOf(deviceNumberSearchTxt));
	}
	...
	private void fillBatchDetails(Device device) {
		WebElementUtils.selectDropDownByVisibleText(createOpenBatchDDwn,
				device.getCreateOpenBatch());
	...
	private void fillDeviceInformation(Device device) {
		...
		WebElementUtils.selectDropDownByVisibleText(applicationTypeDDwn,
				device.getApplicationType());
		WebElementUtils.selectDropDownByVisibleText(subApplicationTypeDDwn,
				device.getSubApplicationType());
		clickNextButton();
	}
	...
	private void fillAddress(Device device) {
		...
		WebElementUtils.enterText(currentAddressLine1Txt,
				currentAddress.getAddressLine1());
		WebElementUtils.selectDropDownByVisibleText(currentCountryCodeDDwn,
				currentAddress.getCountry());
		WebElementUtils.enterText(currentAddressPostalCode,
	...
	private void fillProfile(Device device) {
		...
		WebElementUtils.selectDropDownByVisibleText(titleDDwn,
				client.getTitle());
		...
		WebElementUtils.selectDropDownByVisibleText(genderDDwn,
				client.getGender());
		...
		WebElementUtils.selectDropDownByVisibleText(maritialStatusDDwn,
				client.getMaritialStatus());
		if (device.getAppliedForProduct().equalsIgnoreCase(ProductType.DEBIT)) {
			...
			WebElementUtils.selectDropDownByVisibleText(accountTypeDDwn,
					device.getAccountType());
		}
