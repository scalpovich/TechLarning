...
public class ProgramSetupSteps {
	...
	@Composite(steps = { "When User fills Statement Message Plan for prepaid product",
			...
			"When User fills Device Range section for prepaid product",
			"When user creates a bulk device production request for prepaid",
			"When user processes created bulk device generation request for prepaid",
			"When user processes pre-production batch for prepaid",
			"When user processes device production batch for prepaid",
			"When user processes pin generation batch for prepaid",
			"When user sign out from customer portal",
			"When user is logged in agent portal as admin user",
			"When user fills information to assign program to agency and submits form",
			"When user sign out from agent portal",
	...
	@Composite(steps = {
			"When user creates new device of $deviceType type for new client",
	...
	@Composite(steps = {
			"When user fills General details with product $type and submits the form",
	...
	public void whenUserFillsDevicePlan(String type) {
		...
		/* Below two line code creates template count in DB, it reaches max count 999 on continuous script execution, fetching existing values through excel
		 * This can be used when need arises to execute script for template creation
	...
	public void whenUserFillsBusinessMandatoryFieldsScreen(String type){
		...
		ApplicationBusinessMandatoryFields testDataObject = ApplicationBusinessMandatoryFields.createWithProvider(dataProvider);		
	...
	public void whenUserFillsCreditPlan(){
		...
		// TransactionRulePlan & PaymentPriorityPlan are expected to come from related methods hence fetching data 
		//from them and setting them again below into setTransactionRulePlan & setPaymentPriorityPlan
